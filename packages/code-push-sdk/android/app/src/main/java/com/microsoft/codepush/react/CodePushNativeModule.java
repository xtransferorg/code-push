...
public class CodePushNativeModule extends ReactContextBaseJavaModule {
    ...
    public CodePushNativeModule(ReactApplicationContext reactContext, CodePush codePush, CodePushUpdateManager codePushUpdateManager, CodePushTelemetryManager codePushTelemetryManager, SettingsManager settingsManager) {
        ...
        mBinaryContentsHash = CodePushUpdateUtils.getHashForBinaryContents(reactContext,codePush.getDeploymentKey() ,mCodePush.isDebugMode());
            ...
            new Handler(Looper.getMainLooper()).post(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        // 先暂停接收 JS Ui 事件，避免在 reload 期间接收到事件导致奔溃
                        getReactApplicationContext().getNativeModule(UIManagerModule.class).getUIImplementation().onHostPause();

                        //原有逻辑会绑定ReactRootView，然后回调onReactContextInitialized，导致加载业务 bundle 在 runApplicationReact 之后
                        //所以先把 Attached ReactRootView unmount
                        unmountReactRootView(instanceManager);
    ...
    private void unmountReactRootView(ReactInstanceManager manager) {
        try {

            Field mAttachedRootViewsField = manager.getClass().getDeclaredField("mAttachedReactRoots");
            mAttachedRootViewsField.setAccessible(true);
            Set<ReactRootView> mAttachedRootViews = (Set<ReactRootView>) mAttachedRootViewsField.get(manager);
    ...
    private String getClientUniqueId() {
        if (mClientUniqueId == null) {
            mClientUniqueId = Settings.Secure.getString(getReactApplicationContext().getContentResolver(), Settings.Secure.ANDROID_ID);
        }
