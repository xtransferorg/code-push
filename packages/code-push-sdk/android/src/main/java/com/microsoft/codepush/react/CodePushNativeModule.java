...
import xrn.modules.codepush.NativeCodePushSpec;
...
public class CodePushNativeModule extends NativeCodePushSpec {
    ...
    public CodePushNativeModule(ReactApplicationContext reactContext, CodePush codePush, CodePushUpdateManager codePushUpdateManager, CodePushTelemetryManager codePushTelemetryManager, SettingsManager settingsManager) {
        ...
        mCodePush = codePush;
        mClientUniqueId = null;

        initial(reactContext, codePushUpdateManager, codePushTelemetryManager, settingsManager);
    }

    public void initial(Context context, CodePushUpdateManager codePushUpdateManager, CodePushTelemetryManager codePushTelemetryManager, SettingsManager settingsManager) {
        mSettingsManager = settingsManager;
        mTelemetryManager = codePushTelemetryManager;
        mUpdateManager = codePushUpdateManager;

        // Initialize module state while we have a reference to the current context.
        mBinaryContentsHash = CodePushUpdateUtils.getHashForBinaryContents(context, mCodePush.getDeploymentKey(), mCodePush.isDebugMode());
    }
    ...
    @Override
    public double codePushInstallModeImmediate() {
        return CodePushInstallMode.IMMEDIATE.getValue();
    }

    @Override
    public double codePushInstallModeOnNextRestart() {
        return CodePushInstallMode.ON_NEXT_RESTART.getValue();
    }

    @Override
    public double codePushInstallModeOnNextResume() {
        return CodePushInstallMode.ON_NEXT_RESUME.getValue();
    }

    @Override
    public double codePushInstallModeOnNextSuspend() {
        return CodePushInstallMode.ON_NEXT_SUSPEND.getValue();
    }

    @Override
    public double codePushUpdateStateRunning() {
        return CodePushUpdateState.RUNNING.getValue();
    }

    @Override
    public double codePushUpdateStatePending() {
        return CodePushUpdateState.PENDING.getValue();
    }

    @Override
    public double codePushUpdateStateLatest() {
        return CodePushUpdateState.LATEST.getValue();
    }
    ...
    private void loadBundle() {
        ...
        try {
            mCodePush.clearDebugCacheIfNeeded(resolveReactHost());
        } catch(Exception e) {
...
//                        mCodePush.initializeUpdateAfterRestart();
    ...
    private ReactHost resolveReactHost() throws NoSuchFieldException, IllegalAccessException {
        ReactHost reactHost = mCodePush.getReactHost();
        if (reactHost != null) {
            return reactHost;
        }
        ...
        reactHost = reactApplication.getReactHost();
            ...
            protected Void doInBackground(Void... params) {
                try {
                    ...
                    mUpdateManager.downloadPackage(mutableUpdatePackage, mCodePush.getAssetsBundleFileName(), new DownloadProgressCallback() {
                        ...
                        private  CodePushPatchState currentPatchStep = null;
                        ...
                        @Override
                        public void patchEvent(CodePushPatchState patchStep,int code) {
                                        ...
                                        public void doFrame(long frameTimeNanos) {
                                            if (patchStep!=currentPatchStep) {
                                                ...
                                                getReactApplicationContext()
                                                        ...
                                                        .emit(CodePushConstants.CODE_PUSH_PATCH_STATUS_NAME, CodePushUtils.convertJsonObjectToWritable(new PatchEventInfo(patchStep.getValue(),code).toJson()));
                                            }
    ...
    public void getConfiguration(Promise promise) {
        try {
            ...
            configMap.putString("clientUniqueId", getClientUniqueId());
            configMap.putString("deploymentKey", mCodePush.getDeploymentKey());
            configMap.putString("serverUrl", mCodePush.getServerUrl());
            configMap.putString("commonHash", mCodePush.getBinaryCommonHash());
    ...
    private String getClientUniqueId() {
        if (mClientUniqueId == null) {
            mClientUniqueId = DeviceUtils.INSTANCE.getUniqueDeviceId(getReactApplicationContext(), true);
        }
        
        return mClientUniqueId;
    }
    ...
    public void notifyApplicationReady(Promise promise) {
        try {
            if (mSettingsManager.isPendingHash(mCodePush.getRunningPackageHash())) {
    ...
    public boolean isNativeSyncing() {
        return mCodePush.isIsSyncing();
    }
    ...
    @Override
    public String getIntlResourcePath(String path) {
        // JS only
        return "";
    }
    ...
    public boolean isAssetBundleFileExists() {
        return mCodePush.isAssetBundleFileExists();
    }

    @ReactMethod(isBlockingSynchronousMethod = true)
    public String getBasePackageBundlePath() {
        return mUpdateManager.getBasePackageBundlePath(mCodePush.getAssetsBundleFileName());
    }
