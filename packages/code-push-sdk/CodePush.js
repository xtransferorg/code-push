import { AcquisitionManager as Sdk } from '@xrnjs/code-push-core/script/acquisition-sdk'
...
async function tryReportStatus(statusReport, resumeListener) {
  ...
  try {
    ...
    resumeListener && subscription?.remove?.()
  } catch (e) {
    ...
    if (!resumeListener) {
      ...
      subscription = AppState.addEventListener('change', resumeListener)
    }
  }
}
...
async function syncInternal(
  ...
  try {
    ...
    syncStatusChangeCallback(CodePush.SyncStatus.CHECKING_DONE)
    ...
    const doDownloadAndInstall = async () => {
      ...
      const localPackage = await remotePackage.download(
        ...
        (payload) => {
          ...
          const patchStatus = CodePush.SyncStatus[state]
          if (patchStatus) {
            syncStatusChangeCallback(
              patchStatus,
...
if (NativeCodePush) {
  ...
  Object.assign(CodePush, {
    ...
    InstallMode: {
      ...
      ON_NEXT_RESUME: NativeCodePushState.codePushInstallModeOnNextResume, // Restart the app the next time it is resumed from the background
      ON_NEXT_SUSPEND: NativeCodePushState.codePushInstallModeOnNextSuspend, // Restart the app _while_ it is in the background,
    ...
    UpdateState: {
      RUNNING: NativeCodePushState.codePushUpdateStateRunning,
      PENDING: NativeCodePushState.codePushUpdateStatePending,
      LATEST: NativeCodePushState.codePushUpdateStateLatest,
    },
    DeploymentStatus: {
