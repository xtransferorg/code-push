...
function accessKeyAdd(command: cli.IAccessKeyAddCommand): Promise<void> {
  return sdk
    .addAccessKey(command.name, command.ttl)
    .then((accessKey: AccessKey) => {
      out.text(
        `Successfully created the "${command.name}" access key: ${accessKey.key}`,
      )
      out.text(
...
function accessKeyPatch(command: cli.IAccessKeyPatchCommand): Promise<void> {
  const willUpdateName: boolean =
    isCommandOptionSpecified(command.newName) &&
...
function appAdd(command: cli.IAppAddCommand): Promise<void> {
  ...
  if (normalizedOs === 'ios') {
    ...
    return Promise.reject<void>(
      new Error(
        `"${command.os}" is an unsupported OS. Available options are "ios", "android", "oh", and "windows".`,
      ),
    )
  }
  ...
  return sdk
    .addApp(command.appName, os, platform, command, false)
...
function appModify(command: cli.IAppConfig): Promise<void> {
  const { appName, appKey, repositoryUrl, port } = command
  if (!(appKey || repositoryUrl || port)) {
    throw new Error(
      'At least one property must be specified to modify the app.',
    )
  }
  return sdk
    .modifyApp(appName, {
    ...
    .then(() => {
      out.text(`Successfully modified the "${appName}" app.`)
    })
}
...
function appRename(command: cli.IAppRenameCommand): Promise<void> {
  return sdk
    .renameApp(command.currentAppName, command.newAppName)
    .then((): void => {
      out.text(
...
function listCollaborators(
  command: cli.ICollaboratorListCommand,
): Promise<void> {
  throwForInvalidOutputFormat(command.format)
...
function removeCollaborator(
  command: cli.ICollaboratorRemoveCommand,
): Promise<void> {
  throwForInvalidEmail(command.email)
  ...
  return confirm().then((wasConfirmed: boolean): Promise<void> => {
    if (wasConfirmed) {
      return sdk
        .removeCollaborator(command.appName, command.email)
        .then((): void => {
          out.text(
  ...
  if (command.default) {
    return sdk
      ...
      .then((deployment: Deployment): Promise<void> => {
        out.text(
          'Successfully added the "Staging" and "Production" default deployments:',
        )
        var deploymentListCommand: cli.IDeploymentListCommand = {
          type: cli.CommandType.deploymentList,
...
function deploymentHistoryClear(
  command: cli.IDeploymentHistoryClearCommand,
): Promise<void> {
  return confirm().then((wasConfirmed: boolean): Promise<void> => {
    if (wasConfirmed) {
      return sdk
...
export var deploymentList = (
  ...
  showPackage =
    typeof command.showPackage === 'boolean' ? command.showPackage : true
  return sdk
    .getDeployments(command.appName)
    .then((retrievedDeployments: Deployment[]) => {
      deployments = retrievedDeployments
      if (showPackage) {
        var metricsPromises: Promise<void>[] = deployments.map(
...
function deploymentRemove(
  command: cli.IDeploymentRemoveCommand,
): Promise<void> {
  return confirm(
    ...
    if (wasConfirmed) {
      return sdk
        .removeDeployment(command.appName, command.deploymentName)
        .then((): void => {
          out.text(
...
function deploymentMetrics(command: cli.IDeploymentMetricsCommand) {
  return sdk
    .getDeploymentMetrics(command.appName, command.deploymentName)
    .then((metrics: DeploymentMetrics): void => {
      if (metrics === null) {
        out.text('No deployment metrics found.')
        return
      }
      if (command.format === 'json') {
        out.printJson(metrics)
      } else {
        const headers = ['Label', 'Active', 'Downloaded', 'Failed', 'Installed']
        out.table(
          out.getCommandOutputTableOptions(generateColoredTableTitles(headers)),
          Object.keys(metrics).map((label: string) => {
            const metricsForLabel = metrics[label]
            return [
              label,
              metricsForLabel.active,
              metricsForLabel.downloaded,
              metricsForLabel.failed,
              metricsForLabel.installed,
            ]
          }),
        )
      }
...
function deploymentRename(
  command: cli.IDeploymentRenameCommand,
): Promise<void> {
  return sdk
    .renameDeployment(
      command.appName,
...
export function execute(command: cli.ICommand): Promise<void> {
  ...
  return Promise.resolve().then(() => {
    ...
    switch (command.type) {
      ...
      case cli.CommandType.appModify:
        return appModify(<cli.IAppConfig>command)
      ...
      case cli.CommandType.deploymentHistoryClear:
        return deploymentHistoryClear(
          <cli.IDeploymentHistoryClearCommand>command,
        )
      ...
      case cli.CommandType.batchPatch:
        return batchPatch(<cli.IBatchPatch>command)

      case cli.CommandType.batchRollback:
        return batchRollback(<cli.IBatchRollback>command)
      case cli.CommandType.nativeRelease:
        return nativeRelease(<cli.IReleaseNativeCommand>command)
      case cli.CommandType.nativeUpdateRelease:
        return nativeUpdateRelease(<cli.IReleaseUpdateNativeCommand>command)
      case cli.CommandType.nativeListRelease:
        return nativeListRelease(<cli.IReleaseListNativeCommand>command)

      case cli.CommandType.whoami:
        return whoami(command)

      case cli.CommandType.token:
        return tokenCommand()
...
function getTotalActiveFromDeploymentMetrics(
...
function initiateExternalAuthenticationAsync(
  action: string,
  serverUrl?: string,
): void {
  var message: string
  ...
  if (format === 'json') {
    ...
    out.table(
      out.getCommandOutputTableOptions(
        generateColoredTableTitles(['Name', 'Deployments']),
      ),
...
function getCollaboratorDisplayName(
  ...
  return collaboratorProperties.permission ===
    AccountManager.AppPermission.OWNER
    ? email + chalk.magenta(' (Owner)')
    : email
}

function printCollaboratorsList(
  format: string,
  collaborators: CollaboratorMap,
): void {
  if (format === 'json') {
    var dataSource = { collaborators: collaborators }
    out.printJson(dataSource)
  } else if (format === 'table') {
    out.table(
      out.getCommandOutputTableOptions(
        generateColoredTableTitles(['E-mail Address']),
      ),
      collaborators.map((email) => [
        getCollaboratorDisplayName(email, collaborators[email]),
      ]),
    )
  }
}
...
function printDeploymentHistory(
  ...
  if (command.format === 'json') {
    out.printJson(deploymentHistory)
  } else if (command.format === 'table') {
      ...
      deploymentHistory.map((packageObject: Package) => {
        ...
        row.push(
          packageObject.description
            ? wordwrap(30)(packageObject.description)
            : '',
        )
        row.push(
...
function getPackageMetricsString(obj: Package): string {
  ...
  if (!packageObject || !packageObject.metrics) {
    return (
      chalk.magenta('No installs recorded').toString() + (rolloutString || '')
    )
  }
...
export var release = (command: cli.IReleaseCommand): Promise<void> => {
  ...
  return Promise.resolve().then(() => {
    ...
    var currentCommand: cli.IReleaseCommand = {
      ...
      uuid: command.uuid,
      whiteList: command.whiteList,
      channelReleaseId: command.channelReleaseId,
      bundleName: command.bundleName,
      force: command.force,
      appBinaryTime: command.appBinaryTime,
    }

    var releaseHooksPromise = hooks.reduce(
      (
        accumulatedPromise: Promise<cli.IReleaseCommand>,
        hook: cli.ReleaseHook,
      ) => {
        return accumulatedPromise.then(
          (modifiedCommand: cli.IReleaseCommand) => {
...
export var releaseCordova = (
  command: cli.IReleaseCordovaCommand,
): Promise<void> => {
  ...
  return validateDeployment(command.appName, command.deploymentName)
    .then((): any => {
      ...
      out.text(
        chalk.cyan(`Running "${cordovaCLI} ${cordovaCommand}" command:\n`),
      )
      try {
        execSync(
          [cordovaCLI, cordovaCommand, platform, '--verbose'].join(' '),
          {
            stdio: 'inherit',
          },
        )
  ...
  return (
    validateDeployment(command.appName, command.deploymentName)
      .then((): any => {
        ...
        try {
          var projectPackageJson: any = require(
          ...
          if (!isReactNativeProject) {
            throw new Error(
              'The project in the CWD is not a React Native project.',
            )
          }
        } catch (error) {
          throw new Error(
      ...
      .then(() => {
        if (platform === 'android') {
          ...
          return getiOSHermesEnabled(command.podFile).then(
            (isHermesEnabled) => {
              if (isHermesEnabled) {
                return runHermesEmitBinaryCommand(
                  bundleName,
...
function rollback(command: cli.IRollbackCommand): Promise<void> {
  return confirm('Are you sure?', command.y).then((wasConfirmed: boolean) => {
    if (!wasConfirmed) {
    ...
    return sdk
      .rollback(
        command.appName,
        command.deploymentName,
        command.targetRelease || undefined,
        command.appVersion,
        command.targetUuid,
        command.rollbackUuid,
        command.previous,
      )
      .then((): void => {
        out.text(
...
function batchRollback(command: cli.IBatchRollback): Promise<void> {
  if (!command.channelReleaseId && !command.targetUuid) {
    throw new Error('channelReleaseId and targetUuid must have one entry')
  }
  if (command.channelReleaseId) {
    return sdk
      .batchRollbackWithChannelReleaseId(
        Number(command.channelReleaseId),
        command.rollbackUuid.split(','),
        command.previous,
      )
      .then(() => {
        out.text(`Batch rollback succeeded. ${command.channelReleaseId}`)
      })
  }
  return sdk
    .batchRollback(
      command.targetUuid.split(','),
      command.rollbackUuid.split(','),
      command.previous,
    )
    .then(() => {
      out.text(`Batch rollback succeeded. ${command.targetUuid}`)
    })
}

function nativeRelease(command: cli.IReleaseNativeCommand) {
  return sdk.nativeRelease(command.appKey, command).then((data) => {
    assert(data?.body, 'Expected a non-null response')
    out.printJson(data.body)
  })
}

function nativeUpdateRelease(command: cli.IReleaseUpdateNativeCommand) {
  return sdk.nativeUpdateRelease(command.versionId, command).then((data) => {
    assert(data?.body, 'Expected a non-null response')
    out.printJson(data.body)
  })
}

function nativeListRelease(command: cli.IReleaseListNativeCommand) {
  const { appKey, environment, ...query } = command
  return sdk.nativeListRelease(appKey, environment, query).then((data) => {
    assert(data?.body, 'Expected a non-null response')
    out.printJson(data.body)
  })
}

function batchPatch(command: cli.IBatchPatch): Promise<void> {
  ...
  if (!command.channelReleaseId && !command.uuid) {
    throw new Error('channelReleaseId and uuid must have one entry')
  }
  if (command.channelReleaseId) {
    return sdk
      .batchPatchWithChannelReleaseId(
        Number(command.channelReleaseId),
        packageInfo,
      )
      .then(() => {
        out.text(`Batch modification succeeded. ${command.channelReleaseId}`)
      })
  }
  return sdk.batchPatch(command.uuid.split(','), packageInfo).then(() => {
    out.text(`Batch modification succeeded. ${command.uuid}`)
  })
}
...
function whoami(command: cli.ICommand): Promise<void> {
  return sdk.getAccountInfo().then((account: Account): void => {
    var connectionInfo = deserializeConnectionInfo()
    var accountInfo = `${account.email} - ${connectionInfo.customServerUrl}`
...
function getSdk(
   ...
   */
  Object.getOwnPropertyNames(AccountManager.prototype).forEach(
    (functionName: any) => {
      if (typeof sdk[functionName] === 'function') {
        var originalFunction = sdk[functionName]
        sdk[functionName] = function () {
          var maybePromise: Promise<any> = originalFunction.apply(
            sdk,
            arguments,
          )
...
function tokenCommand(): Promise<void> {
  const conn = deserializeConnectionInfo()
  if (!conn || !conn.accessKey) {
    out.text('No accessKey found in config file.')
    return Promise.resolve()
  }
  out.text(conn.accessKey)
  return Promise.resolve()
}
