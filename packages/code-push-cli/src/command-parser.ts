...
export function showHelp(showRootDescription?: boolean): void {
  if (!wasHelpShown) {
    if (showRootDescription) {
      ...
      console.log(
        chalk.cyan(
          '/ /__/ _ \\/ _  / -_)' + chalk.green(' ___/ // (_-</ _ \\'),
        ),
      )
      console.log(
        chalk.cyan(
          '\\___/\\___/\\_,_/\\__/' + chalk.green('_/   \\_,_/___/_//_/'),
        ) +
          '    CLI v' +
          packageJson.version,
      )
...
function accessKeyPatch(commandName: string, yargs: yargs.Argv): void {
  ...
  yargs
    ...
    .example(
      'access-key ' +
        commandName +
        ' "Key for build server" --name "Key for CI machine"',
...
function accessKeyList(commandName: string, yargs: yargs.Argv): void {
  ...
  yargs
    ...
    .example(
      'access-key ' + commandName,
      'Lists your access keys in tabular format',
    )
    .example(
    ...
    .option('format', {
      ...
      demand: false,
      description:
        'Output format to display your access keys with ("json" or "table")',
      type: 'string',
    })
...
function appList(commandName: string, yargs: yargs.Argv): void {
  ...
  yargs
    ...
    .example(
      'app ' + commandName + ' --format json',
      'List your apps in JSON format',
    )
    .option('format', {
      default: 'table',
      demand: false,
      description:
        'Output format to display your apps with ("json" or "table")',
      type: 'string',
    })
...
function listCollaborators(commandName: string, yargs: yargs.Argv): void {
  ...
  yargs
    .usage(
      USAGE_PREFIX + ' collaborator ' + commandName + ' <appName> [options]',
    )
    .demand(/*count*/ 1, /*max*/ 1) // Require exactly one non-option arguments
    .example(
    ...
    .option('format', {
      ...
      demand: false,
      description:
        'Output format to display collaborators with ("json" or "table")',
      type: 'string',
    })
...
function deploymentList(commandName: string, yargs: yargs.Argv): void {
  ...
  yargs
    ...
    .option('format', {
      ...
      demand: false,
      description:
        'Output format to display your deployments with ("json" or "table")',
      type: 'string',
    })
    .option('displayKeys', {
      alias: 'k',
    ...
    .option('showPackage', {
      demand: false,
      description: 'Display installation information',
      type: 'boolean',
    })
  addCommonConfiguration(yargs)
}
  ...
  yargs
    .usage(
      USAGE_PREFIX +
        ' deployment ' +
        commandName +
        ' <appName> <deploymentName> [options]',
    )
    .demand(/*count*/ 2, /*max*/ 2) // Require exactly two non-option arguments
    .example(
      'deployment ' + commandName + ' MyApp MyDeployment',
    ...
    .option('format', {
      ...
      demand: false,
      description:
        'Output format to display the release history with ("json" or "table")',
      type: 'string',
    })
...
const release = (yargs: yargs.Argv) => {
  ...
  yargs
    ...
    .option('deploymentName', {
      alias: 'd',
      default: 'Production', // Default to the Production deployment if the user doesn't specify one.
      demand: false,
    ...
    .option('whiteList', {
      ...
      demand: false,
      description:
        'Grayscale whitelist. Users in the whitelist will receive updates at one point',
      type: 'string',
    })
    .option('uuid', {
      default: null,
      demand: false,
      description:
        'Association ID: Associate an external record with a package one by one for query and rollback',
      type: 'string',
    })
    .option('channelReleaseId', {
      ...
      demand: false,
      description:
        'Id of the channel publishing platform, which is used to associate publishing requirements',
      type: 'string',
    })
    .option('force', {
      alias: 'f',
      ...
      demand: false,
      description:
        'Force override the online version, effective only when using the release-v0 command.',
      type: 'boolean',
    })
    .option('bundleName', {
      default: null,
      demand: false,
      description: 'Current bundle name',
      type: 'string',
    })
    .option('appBinaryTime', {
      default: null,
      demand: false,
...
var argv = yargs
    ...
    (yargs: yargs.Argv) => {
      ...
      yargs
        ...
        .command(
          ...
          (yargs: yargs.Argv) => accessKeyRemove('remove', yargs),
        )
        ...
        .check(
          (argv: any, aliases: { [aliases: string]: string }): any =>
            isValidCommand,
        ) // Report unrecognized, non-hyphenated command category.
  ...
  .command('app', 'View and manage your CodePush apps', (yargs: yargs.Argv) => {
    ...
    yargs
      ...
      .command(
        ...
        (yargs: yargs.Argv) => appRemove('remove', yargs),
      )
      ...
      .command(
        'list',
        'Lists the apps associated with your account',
        (yargs: yargs.Argv) => appList('list', yargs),
      )
      .command(
        'ls',
        'Lists the apps associated with your account',
        (yargs: yargs.Argv) => appList('ls', yargs),
      )
      .command(
        'modify',
        'Change the bundle port, repository address, and which native app it belongs to',
        (yargs: yargs.Argv) => {
          isValidCommand = true
          yargs
            .usage(USAGE_PREFIX + ' app modify <appName>')
            .option('port', {
              alias: 'p',
              type: 'number',
            })
            ...
            .example('app modify MyApp --port 8081', 'Changes the port to 8081')
  ...
  .command(
    'collaborator',
    'View and manage app collaborators',
    (yargs: yargs.Argv) => {
      isValidCommandCategory = true
      yargs
        ...
        .command(
          ...
          (yargs: yargs.Argv) => removeCollaborator('remove', yargs),
        )
        .command(
          'rm',
          'Remove a collaborator from an app',
          (yargs: yargs.Argv) => removeCollaborator('rm', yargs),
        )
        .command(
          'list',
          'List the collaborators for an app',
          (yargs: yargs.Argv) => listCollaborators('list', yargs),
        )
        .command(
          'ls',
          'List the collaborators for an app',
          (yargs: yargs.Argv) => listCollaborators('ls', yargs),
        )
        .check(
          (argv: any, aliases: { [aliases: string]: string }): any =>
            isValidCommand,
  ...
  .command(
    'debug',
    'View the CodePush debug logs for a running app',
    (yargs: yargs.Argv) => {
      isValidCommandCategory = true
      isValidCommand = true
      yargs
        ...
        .example(
          'debug ios',
          'View the CodePush debug logs for the iOS simulator',
        )
      ...
      yargs
        ...
        .command(
          ...
          (yargs: yargs.Argv): void => {
            ...
            yargs
              .usage(
                USAGE_PREFIX + ' deployment add <appName> [deploymentName]',
              )
              ...
              .option('default', {
                ...
                demand: false,
                description:
                  'Add the default "Staging" and "Production" deployments',
                type: 'boolean',
              })
              .example(
        ...
        .command(
          ...
          (yargs: yargs.Argv) => deploymentRemove('remove', yargs),
        )
        ...
        .command(
          ...
          (yargs: yargs.Argv) => deploymentList('list', yargs),
        )
        .command(
          'ls',
          'List the deployments associated with an app',
          (yargs: yargs.Argv) => deploymentList('ls', yargs),
        )
        .command(
          'history',
        ...
        .command(
          'metrics',
          'Display metrics for the corresponding deployment.',
          (yargs: yargs.Argv) => {
            isValidCommand = true
            yargs
              .usage(
              ...
              .example(
                'deployment metrics' + ' MyApp',
                'Lists the deployments for app "MyApp" in tabular format',
              )
              .example(
                'deployment metrics' + ' MyApp --format json',
                'Lists the deployments for app "MyApp" in JSON format',
              )
              .option('format', {
                default: 'json',
        ...
        .command(
          'h',
          'Display the release history for a deployment',
          (yargs: yargs.Argv) => deploymentHistory('h', yargs),
        )
        .check(
          (argv: any, aliases: { [aliases: string]: string }): any =>
            isValidCommand,
  ...
  .command(
    'login',
    'Authenticate in order to begin managing your apps',
    (yargs: yargs.Argv) => {
      isValidCommandCategory = true
      isValidCommand = true
      yargs
    ...
    (yargs: yargs.Argv) => {
      ...
      yargs
        ...
        .option('uuid', {
          ...
          demand: false,
          description:
            'Association ID: Associate an external record with a package one by one for query and rollout',
          type: 'string',
        })
  ...
  .command(
    'register',
    'Register a new Mobile Center account',
    (yargs: yargs.Argv) => {
      isValidCommandCategory = true
      isValidCommand = true
      yargs
  ...
  .command('release', 'Release an update to an app deployment', release)
  .command(
    'release-cordova',
  ...
  .command(
    ...
    (yargs: yargs.Argv) => {
      ...
      yargs
        .usage(
          USAGE_PREFIX +
            ' rollback <appName> <deploymentName> <appVersion> [options]',
        )
        .demand(/*count*/ 2, /*max*/ 2) // Require exactly two non-option arguments
        .example(
        ...
        .option('y', {
          ...
          demand: false,
          description:
            'Automatically acknowledge rollback, commonly used in ci processes',
          type: 'boolean',
        })
        .option('appVersion', {
          alias: 'v1',
          demand: true,
          description:
            'Roll back to the version associated with a particular app',
          type: 'string',
        })
        .option('targetUuid', {
          demand: false,
          description:
            'Roll back to the batch version associated with the specified association ID',
          type: 'string',
        })
        .option('rollbackUuid', {
          demand: false,
          description:
            'Used to determine the uuid associated with the package after the current app is rolled back',
          type: 'string',
        })
        .option('previous', {
          demand: false,
          default: false,
          description:
            'Roll back to the previous version with the specified associated ID',
          type: 'boolean',
        })
  ...
  .command(
    'batch',
    'Batch tasks, applicable to rollback and unified control',
    (yargs: yargs.Argv) => {
      isValidCommandCategory = true
      yargs
        .usage(USAGE_PREFIX + ' batch <command>')
        .command('patch', 'Batch patch package', (yargs: yargs.Argv) => {
          isValidCommand = true
          yargs
            .usage(USAGE_PREFIX + ' batch patch')
            .example(
              'batch patch --channelReleaseId 10345 --rollout 30%',
              'Use the channel to publish id10345 batch to modify the gray scale of hot updates to 30%',
            )
            .example(
              'batch patch --uuid "x1,x2" --rollout 30%',
              'Modify the gray scale to 30% with a hot updated uuid',
            )
            .option('uuid', {
              default: null,
              demand: false,
              description:
                'Association ID: Associate an external record with a package',
              type: 'string',
            })
            .option('channelReleaseId', {
              default: null,
        ...
        .command('rollback', 'Batch rollback package', (yargs: yargs.Argv) => {
          isValidCommand = true
          yargs
            .usage(
              USAGE_PREFIX +
                ' batch rollback --targetUuid "x1,x2" --rollbackUuid "y1,y2"',
            )
            .option('targetUuid', {
              demand: false,
              description:
  ...
  .command('native', 'Manage a new release to an app', (yargs: yargs.Argv) => {
    isValidCommandCategory = true
    yargs
      .usage(USAGE_PREFIX + ' native release [options]')
      .command(
        'release',
        ...
        (yargs: yargs.Argv) => {
          ...
          yargs
            .option('appKey', {
              demandOption: true,
              description: 'App key of the app to publish the release to',
              type: 'string',
            })
            .option('versionName', {
              demandOption: true,
            ...
            .option('channel', {
              demandOption: true,
              description: 'Channel of the release',
              type: 'string',
            })
            .option('environment', {
              demandOption: true,
              description: 'Environment of the release. "dev" or "prod"',
              type: 'string',
            })
            .option('appFormat', {
              demandOption: true,
              description: 'appFormat of the release.',
              type: 'string',
            })
            .option('buildType', {
              description: 'Build type of the release. "debug" or "release"',
              type: 'string',
            })
            .option('isBackwardCompatible', {
              description: 'Is the release backward compatible',
              type: 'boolean',
            })
            .option('updateType', {
              description:
            ...
            .option('onlyApplyVersion', {
              description: 'Only apply the release to the specified version',
              type: 'string',
            })
            .option('notOnlyApplyVersion', {
              description:
                'Not only apply the release to the specified version',
              type: 'string',
            })
            .example(
              'native release --appKey "xxab12bc" --versionName "1.0.0" --changeLog "Initial release" --downloadUrl "https://example"',
              'release a new version of the app, with version name 1.0.0, changelog "Initial release" and download url "https://example"',
            )
    ...
    yargs
      .usage(USAGE_PREFIX + ' native update-release [options]')
      .command(
        'update-release',
        ...
        (yargs: yargs.Argv) => {
          ...
          yargs
            .option('versionId', {
              demandOption: true,
              description: 'Version id of the release to update',
              type: 'string',
            })
            .option('rollout', {
              description: 'Rollout of the release',
              type: 'string',
            })
            .option('isBackwardCompatible', {
              description: 'Is the release backward compatible',
            ...
            .option('whiteList', {
              description: 'Whitelist of the release',
              type: 'string',
            })
            .option('updateType', {
              description: 'Update type of the release. "Force" or "Silent"',
              type: 'string',
            })
            .option('changeLog', {
              description: 'Changelog of the release',
            ...
            .example(
              'native update-release --versionId "xxab12bc" --versionName "1.0.0" --changeLog "Initial release" --downloadUrl "https://example"',
              'update an existing release with version id xxab12bc, with version name 1.0.0, changelog "Initial release" and download url "https://example"',
            )
          addCommonConfiguration(yargs)
        },
      )
    ...
    yargs
      .usage(USAGE_PREFIX + ' native list-release [options]')
      .command('list-release', 'List all release', (yargs: yargs.Argv) => {
        isValidCommand = true
        yargs
          .option('appKey', {
            demandOption: true,
            description: 'App key of the app to list the releases for',
            type: 'string',
          })
          .option('environment', {
            demandOption: true,
          ...
          .option('page', {
            description: 'Page number of the releases',
            type: 'number',
          })
          .option('limit', {
            description: 'Limit of the releases',
            type: 'number',
          })
          .option('versionName', {
            description: 'Version name of the release',
          ...
          .example(
            'native list-release --appKey "xxab12bc" --environment "production"',
            'list all releases for the app with app key xxab12bc in the production environment',
          )
        addCommonConfiguration(yargs)
      })
  ...
  .command(
    'token',
    'Display the access key from the config file',
    (yargs: yargs.Argv) => {
      isValidCommandCategory = true
      isValidCommand = true
      yargs
        ...
        .demand(/*count*/ 0, /*max*/ 0)
        .example('token', 'Display the access key from the config file')
      addCommonConfiguration(yargs)
    },
  )
  .alias('v', 'version')
    ...
    switch (arg0) {
      ...
      case 'app':
        switch (arg1) {
          ...
          case 'modify':
            if (arg2) {
              cmd = { type: cli.CommandType.appModify }
              var appModifyCommand = <cli.IAppAddCommand>cmd
              appModifyCommand.appName = arg2
        ...
        switch (arg1) {
          ...
          case 'ls':
            if (arg2) {
              ...
              ;(<cli.ICollaboratorListCommand>cmd).format = argv[
                'format'
              ] as string
            }
            break
        ...
        switch (arg1) {
          ...
          case 'ls':
            if (arg2) {
              ...
              deploymentListCommand.showPackage = argv['showPackage'] as boolean
            }
            break
          ...
          case 'metrics':
            if (arg2 && arg3) {
              ...
              var deploymentMetricsCommand = <cli.IDeploymentHistoryCommand>cmd

              deploymentMetricsCommand.appName = arg2
              deploymentMetricsCommand.deploymentName = arg3
              deploymentMetricsCommand.format = argv['format'] as string
            }
            break
        }
        break
      ...
      case 'patch':
        if (arg1 && arg2) {
          ...
          patchCommand.whiteList = argv['whiteList'] as string
        }
        break
      ...
      case 'promote':
        if (arg1 && arg2 && arg3) {
          ...
          deploymentPromoteCommand.rollout = getRolloutValue(
            argv['rollout'] as string,
          )
      ...
      case 'release':
        if (arg1 && arg2 && arg3) {
          ...
          releaseCommand.uuid = argv['uuid'] as string
          releaseCommand.whiteList = argv['whiteList'] as string
          releaseCommand.channelReleaseId = argv['channelReleaseId'] as string
          releaseCommand.force = argv['force'] as boolean
          releaseCommand.bundleName = argv['bundleName'] as string
          releaseCommand.appBinaryTime = argv['appBinaryTime'] as string
        }
        break
      ...
      case 'release-cordova':
        if (arg1 && arg2) {
          ...
          releaseCordovaCommand.deploymentName = argv[
            'deploymentName'
          ] as string
          ...
          releaseCordovaCommand.rollout = getRolloutValue(
            argv['rollout'] as string,
          )
          releaseCordovaCommand.appStoreVersion = argv[
            'targetBinaryVersion'
          ] as string
          releaseCordovaCommand.isReleaseBuildType = argv[
            'isReleaseBuildType'
          ] as boolean
          releaseCordovaCommand.privateKeyPath = argv[
            'privateKeyPath'
          ] as string
        }
        break
      ...
      case 'release-react':
        if (arg1 && arg2) {
          ...
          releaseReactCommand.appStoreVersion = argv[
            'targetBinaryVersion'
          ] as string
          ...
          releaseReactCommand.plistFilePrefix = argv[
            'plistFilePrefix'
          ] as string
          releaseReactCommand.rollout = getRolloutValue(
            argv['rollout'] as string,
          )
          releaseReactCommand.privateKeyPath = argv['privateKeyPath'] as string
          releaseReactCommand.sourcemapOutput = argv[
            'sourcemapOutput'
          ] as string
          releaseReactCommand.sourcemapOutputDir = argv[
            'sourcemapOutputDir'
          ] as string
          ...
          releaseReactCommand.extraHermesFlags = argv[
            'extraHermesFlags'
          ] as string[]
        }
        break
      ...
      case 'rollback':
        if (arg1 && arg2) {
          ...
          rollbackCommand.appVersion = argv['appVersion'] as string
          rollbackCommand.targetUuid = argv['targetUuid'] as string
          rollbackCommand.rollbackUuid = argv['rollbackUuid'] as string
          rollbackCommand.y = argv['y'] as boolean
          rollbackCommand.previous = argv['previous'] as boolean
        }
        break
      ...
      case 'batch':
        switch (arg1) {
          case 'patch':
            if (argv['uuid'] || argv['channelReleaseId']) {
              cmd = { type: cli.CommandType.batchPatch }

              var batchPatchCommand = <cli.IBatchPatch>cmd

              batchPatchCommand.uuid = argv['uuid'] as string
              batchPatchCommand.channelReleaseId = argv[
                'channelReleaseId'
              ] as string
              batchPatchCommand.description = argv['description']
                ? backslash(argv['description'] as string)
                : null
              batchPatchCommand.disabled = argv['disabled'] as boolean
              batchPatchCommand.mandatory = argv['mandatory'] as boolean
              batchPatchCommand.rollout = getRolloutValue(
                argv['rollout'] as string,
              )
              batchPatchCommand.whiteList = argv['whiteList'] as string
            }
            break
          case 'rollback':
            if (
              (argv['targetUuid'] || argv['channelReleaseId']) &&
              argv['rollbackUuid']
            ) {
              cmd = { type: cli.CommandType.batchRollback }

              var batchRollbackCommand = <cli.IBatchRollback>cmd

              batchRollbackCommand.channelReleaseId = argv[
                'channelReleaseId'
              ] as string
              batchRollbackCommand.targetUuid = argv['targetUuid'] as string
              batchRollbackCommand.rollbackUuid = argv['rollbackUuid'] as string
              batchRollbackCommand.previous = argv['previous'] as boolean
            }
            break
        }
        break
      ...
      case 'native':
        switch (arg1) {
          case 'release':
            cmd = { type: cli.CommandType.nativeRelease }
            var nativeReleaseCommand = <cli.IReleaseNativeCommand>cmd
            nativeReleaseCommand.appKey = argv['appKey'] as string
            nativeReleaseCommand.versionName = argv['versionName'] as string
            nativeReleaseCommand.versionNumber = argv['versionNumber'] as string
            nativeReleaseCommand.changeLog = argv['changeLog'] as string
            nativeReleaseCommand.downloadUrl = argv['downloadUrl'] as string
            nativeReleaseCommand.buildType = argv['buildType'] as BuildType
            nativeReleaseCommand.isBackwardCompatible = argv[
              'isBackwardCompatible'
            ] as boolean
            nativeReleaseCommand.environment = argv[
              'environment'
            ] as Environment
            nativeReleaseCommand.updateType = argv['updateType'] as UpdateType
            nativeReleaseCommand.channel = argv['channel'] as string
            nativeReleaseCommand.onlyApplyVersion = argv[
              'onlyApplyVersion'
            ] as string
            nativeReleaseCommand.status = argv['status'] as string
            nativeReleaseCommand.appFormat = argv['appFormat'] as string
            nativeReleaseCommand.notOnlyApplyVersion = argv[
              'notOnlyApplyVersion'
            ] as string
            break
          case 'update-release':
            cmd = { type: cli.CommandType.nativeUpdateRelease }
            var nativeReleaseUpdateCommand = <cli.IReleaseUpdateNativeCommand>(
              cmd
            )
            nativeReleaseUpdateCommand.versionId = argv['versionId'] as string
            nativeReleaseUpdateCommand.changeLog = argv['changeLog'] as string
            nativeReleaseUpdateCommand.rollout = getRolloutValue(
              argv['rollout'] as string,
            )
            if (nativeReleaseUpdateCommand.rollout === null) {
              nativeReleaseUpdateCommand.rollout = undefined // undefined means "don't update"
            }
            nativeReleaseUpdateCommand.status = argv['status'] as string
            nativeReleaseUpdateCommand.whiteList = argv['whiteList'] as string
            nativeReleaseUpdateCommand.updateType = argv[
              'updateType'
            ] as UpdateType
            nativeReleaseUpdateCommand.isBackwardCompatible = argv[
              'isBackwardCompatible'
            ] as boolean
            nativeReleaseUpdateCommand.onlyApplyVersion = argv[
              'onlyApplyVersion'
            ] as string
            nativeReleaseUpdateCommand.notOnlyApplyVersion = argv[
              'notOnlyApplyVersion'
            ] as string
            break
          case 'list-release':
            cmd = { type: cli.CommandType.nativeListRelease }
            var nativeListReleaseCommand = <cli.IReleaseListNativeCommand>cmd
            nativeListReleaseCommand.appKey = argv['appKey'] as string
            nativeListReleaseCommand.environment = argv[
              'environment'
            ] as Environment
            nativeListReleaseCommand.page = argv['page'] as number
            nativeListReleaseCommand.limit = argv['limit'] as number
            nativeListReleaseCommand.versionName = argv['versionName'] as string
            nativeListReleaseCommand.buildType = argv['buildType'] as BuildType
            nativeListReleaseCommand.channel = argv['channel'] as string
            nativeListReleaseCommand.updateType = argv[
              'updateType'
            ] as UpdateType
            break
        }
      ...
      case 'whoami':
        ...
        break
      case 'token':
        cmd = { type: cli.CommandType.token }
        break
    }
