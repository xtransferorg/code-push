...
export function getReactNativeProjectAppVersion(
  ...
  if (command.platform === 'ios') {
    ...
    if (parsedPlist && parsedPlist.CFBundleShortVersionString) {
      if (/MARKETING_VERSION/i.test(parsedPlist.CFBundleShortVersionString)) {
        try {
          ...
          const xcodeVersionRegex =
            /Release[\s\S]*MARKETING_VERSION = (\d+\.\d+\.\d+)/gm
          let xcodeVersion
          let match
    ...
    try {
      var appxManifestContainingFolder: string = path.join(
        'windows',
        projectName,
      )
      var appxManifestContents: string = fs
        .readFileSync(
          path.join(appxManifestContainingFolder, 'Package.appxmanifest'),
        )
        .toString()
    } catch (err) {
      throw new Error(
    ...
    return new Promise<string>((resolve, reject) => {
      xml2js.parseString(
        ...
        (err: Error, parsedAppxManifest: any) => {
          if (err) {
            reject(
              new Error(
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      composeSourceMapsProcess.on('close', (exitCode: number) => {
        ...
        // Delete the HBC sourceMap, otherwise it will be included in '@xrnjs/code-push-core' bundle as well
...
export async function getHermesEnabled(gradleFile?: string): Promise<boolean> {
  ...
  try {
    ...
    throw new Error(
      `An error occurred while processing the Gradle files: ${error.message}`,
    )
  }
}
