...
class AccountManager {
  ...
  public isAuthenticated(throwIfUnauthorized?: boolean): Promise<boolean> {
    return Promise<any>((resolve, reject, notify) => {
      var request: superagent.Request<any> = superagent.get(
  ...
  public modifyApp(appName: string, config: AddAppConfig = {}) {
    return this.patch(
      urlEncode`/apps/${this.appNameParam(appName)}/updateInfo`,
  ...
  public renameDeployment(
    ...
    return this.patch(
      urlEncode`/apps/${this.appNameParam(appName)}/deployments/${oldDeploymentName}`,
  ...
  public getDeploymentMetrics(
    ...
    return this.get(
      urlEncode`/apps/${this.appNameParam(appName)}/deployments/${deploymentName}/metrics`,
  ...
  public release(
    ...
    return Promise<Package>((resolve, reject, notify) => {
      ...
      var request: superagent.Request<any> = superagent.post(
        this._serverUrl +
          urlEncode`/apps/${this.appNameParam(appName)}/deployments/${deploymentName}/release`,
      )
      if (this._proxy) (<any>request).proxy(this._proxy)
      this.attachCredentials(request)
      ...
      getPackageFilePromise.then((packageFile: PackageFile) => {
        ...
        request
          ...
          .end((err: any, res: superagent.Response) => {
            ...
            if (res.ok) {
              resolve(<Package>res?.body?.package)
            } else {
  ...
  public rollback(
    ...
    // targetRelease 和 uuid 是相互冲突的，都是用于指定回滚到的版本
    if (targetUuid) {
      const targetUuids = targetUuid.split(',')
      const rollbackUuids = rollbackUuid?.split(',') || []
      return this.batchRollback(targetUuids, rollbackUuids, previous)
    }
    return this.post(
      urlEncode`/apps/${this.appNameParam(appName)}/deployments/${deploymentName}/rollback/${targetRelease || ``}`,
      /*requestBody=*/ JSON.stringify({ appVersion }),
      /*expectResponseBody=*/ false,
    ).then(() => null)
  }

  public batchPatch(uuid: string[], packageInfo: PackageInfo) {
    return this.patch(
      urlEncode`/apps/package/patch`,
      JSON.stringify({ uuid, packageInfo }),
    ).then(() => null)
  }

  public batchPatchWithChannelReleaseId(
    channelReleaseId: number,
    packageInfo: PackageInfo,
  ) {
    return this.patch(
      urlEncode`/apps/package/patch`,
      JSON.stringify({ channelReleaseId, packageInfo }),
    ).then(() => null)
  }
  ...
  public batchRollback(
    ...
    return this.post(
      urlEncode`/apps/package/rollback`,
      /*requestBody=*/ JSON.stringify({ targetUuids, rollbackUuids, previous }),
      /*expectResponseBody=*/ false,
    ).then(() => null)
  }

  public batchRollbackWithChannelReleaseId(
    channelReleaseId: number,
    rollbackUuids: string[],
    previous?: boolean,
  ) {
  ...
  public nativeRelease(appKey: string, release: NativeRelease): Promise<void> {
    return this.post(
      urlEncode`/xrn/app/publish`,
      JSON.stringify({
        app_key: appKey,
        download_url: release.downloadUrl,
        version_name: release.versionName,
        version_number: release.versionNumber,
        is_backward_compatible: release.isBackwardCompatible,
        environment: release.environment,
        build_type: release.buildType,
        update_type: release.updateType,
        changelog: release.changeLog,
        channel: release.channel,
        status: release.status,
        app_format: release.appFormat,
        only_apply_version: release.onlyApplyVersion,
        not_only_apply_version: release.notOnlyApplyVersion,
      }),
    )
  }

  public nativeUpdateRelease(
    versionId: string,
    updateRelease: NativeUpdateRelease,
  ): Promise<void> {
    return this.patch(
      urlEncode`/xrn/app/update_publish`,
      JSON.stringify({
        version_id: versionId,
        download_url: updateRelease.downloadUrl,
        is_backward_compatible: updateRelease.isBackwardCompatible,
        update_type: updateRelease.updateType,
        changelog: updateRelease.changeLog,
        rollout: updateRelease.rollout,
        white_list: updateRelease.whiteList,
        status: updateRelease.status,
        only_apply_version: updateRelease.onlyApplyVersion,
        not_only_apply_version: updateRelease.notOnlyApplyVersion,
      }),
    )
  }

  public nativeListRelease(
    appKey: string,
    environment: Environment,
    query: Omit<NativeListRelease, 'appKey' | 'environment'>,
  ) {
    return this.get(
      urlEncode`/xrn/app/query_version/${environment}/${appKey}?${new URLSearchParams(query).toString()}`,
    )
  }
  ...
  private packageFileFromPath(filePath: string): Promise<PackageFile> {
    ...
    if (fs.lstatSync(filePath).isDirectory()) {
      getPackageFilePromise = Promise<PackageFile>(
        (
  ...
  private makeApiRequest(
    ...
    return Promise<JsonResponse>((resolve, reject, notify) => {
      var request: superagent.Request<any> = (<any>superagent)[method](
