...
appsRouter.get(
  '/package/:uuids',
  async (req: Req<{ uuids: string }>, res, next) => {
    const { logger, params } = req
    const { uuids } = params
    logger.info('find package item', { uuids })
    try {
      const data = await packageManager.findPackageLifeCycleWithUuid(
        uuids.split(','),
      )
      res.send(data)
    } catch (e) {
      if (e instanceof AppError) {
        logger.info('find package error', { uuids, error: e.message })
        res.status(406).send(e.message)
      } else {
        next(e)
      }
...
appsRouter.post(
  ...
  async (
    ...
    try {
      let data: Awaited<
        ReturnType<
          typeof packageManager.findPackageLifeCycleWithChannelReleaseId
        >
      > = []
      if (channelReleaseId) {
        data = await packageManager.findPackageLifeCycleWithChannelReleaseId(
          channelReleaseId,
          previous,
        )
      } else if (targetUuids.length !== rollbackUuids.length) {
        throw new AppError('targetUuid 和 rollbackUuid 没有一一对应')
      } else {
        data = await packageManager.findPackageLifeCycleWithUuid(
          targetUuids,
          previous,
        )
      ...
      // 通过 transaction 将回滚操作放在一个事务中
      await sequelize.transaction(async (t) => {
        for (let i = 0; i < data.length; i++) {
          const item = data[i]
          const rollbackUuid = rollbackUuids[i]
          await rollback(
            {
              params: {
                appName: item.appName,
                deploymentName: item.env,
                label: item.packageItem.label,
                deploymentVersionId: item.packageItem.deployment_version_id,
      ...
      logger.info('rollback success', {
        ...
        rollbackUuids: rollbackUuids,
      })
      res.send('ok')
    } catch (e) {
      if (e instanceof AppError) {
        logger.info('rollback package error', {
          targetUuids,
          rollbackUuids,
          error: e.message,
        })
...
appsRouter.get(
  ...
  (req: Req<{ appName: string }>, res, next) => {
    ...
    accountManager
      ...
      .then((col) => {
        return deploymentsManager.listDeployments(col.appid)
      })
      .then((data) => {
...
appsRouter.get(
  ...
  (req: Req<{ appName: string; deploymentName: string }>, res, next) => {
    ...
    accountManager
      .collaboratorCan(uid, appName, logger)
      .then((col) => {
        return deploymentsManager.findDeploymentByName(
          deploymentName,
          col.appid,
          logger,
        )
      ...
      .then((deploymentInfo) => {
        ...
        res.send({
          deployment: deploymentsManager.listDeployment(deploymentInfo),
        })
        return true
      })
      .catch((e) => {
...
// 创建 deployments key，默认会创建 Production, Staging
appsRouter.post(
  '/:appName/deployments',
  checkToken,
  (req: Req<{ appName: string }, { name: string }>, res, next) => {
    ...
    accountManager
      ...
      .then((col) => {
        return deploymentsManager.addDeployment(name, col.appid, uid)
      })
      .then((data) => {
...
appsRouter.get(
  ...
  (req: Req<{ appName: string; deploymentName: string }>, res, next) => {
    ...
    accountManager
      ...
      .then((deploymentInfo) => {
        return deploymentsManager.getAllPackageIdsByDeploymentsId(
          deploymentInfo.id,
        )
      })
      .then((packagesInfos) => {
...
appsRouter.patch(
  ...
  checkToken,
  (
    req: Req<{ appName: string; deploymentName: string }, { name: string }>,
    res,
    next,
  ) => {
    const { logger, params, body } = req
    ...
    accountManager
      .ownerCan(uid, appName, logger)
      .then((col) => {
        return deploymentsManager.renameDeploymentByName(
          deploymentName,
          col.appid,
          name,
        )
...
appsRouter.delete(
  ...
  (req: Req<{ appName: string; deploymentName: string }>, res, next) => {
    ...
    accountManager
      .ownerCan(uid, appName, logger)
      .then((col) => {
        return deploymentsManager.deleteDeploymentByName(
          deploymentName,
          col.appid,
        )
      })
        ...
        return deploymentsManager
          ...
          .then((deploymentInfo) => {
            ...
            const deploymentVersionId =
              deploymentInfo.last_deployment_version_id
            return packageManager
...
appsRouter.patch(
  ...
  async (
    req: Req<
      {},
      { uuid?: string[]; channelReleaseId?: string; packageInfo: PackageInfo }
    >,
    res,
    next,
  ) => {
    const { logger, body } = req
    const { uuid, channelReleaseId, packageInfo } = body
    const uid = req.users.id
    logger.info('try modify release package', {
      uid,
      body: JSON.stringify(body),
    ...
    try {
      if (channelReleaseId) {
        logger.info('modify release by ' + channelReleaseId, packageInfo)
        await packageManager.modifyRelease(channelReleaseId, packageInfo)
      } else {
        const data = await packageManager.findPackageLifeCycleWithUuid(uuid)
        if (isEmpty(data)) {
          throw new AppError('no corresponding hot update found')
        }
        await sequelize.transaction(async (t) => {
          for (const item of data) {
            await packageManager.modifyReleasePackage(
              item.packageItem.id,
              packageInfo,
              t,
            )
            ...
            if (config.common.updateCheckCache) {
              delay(2500).then(() => {
                clientManager.clearUpdateCheckCache(
                  item.deployment.deployment_key,
                  '*',
                  '*',
                  '*',
                  logger,
  ...
  (
    ...
    accountManager
      ...
      .then((col) => {
        ...
        return Promise.all([
          deploymentsManager.findDeploymentByName(
            sourceDeploymentName,
            appId,
            logger,
          ),
  ...
  try {
    if (appVersion) {
      const deploymentVersion =
        await deploymentsManager.findDeploymentVersionWithAppVersion(
          appVersion,
          { uid, appName, deploymentName, logger },
        )
      if (!deploymentVersion) {
        throw new AppError(`没有找到 ${appVersion} 发布记录`)
      }
      req.params.deploymentVersionId = deploymentVersion.id
    }
    await rollback(req)
    res.send('ok')
  } catch (e) {
...
appsRouter.post(
  '/:appName/deployments/:deploymentName/rollback',
  checkToken,
  rollbackCore,
)
...
/**
 * 创建 APP，默认会对此 APP 创建对应的 Deployment 的 Production 和 Staging
 * code-push app add APP_NAME ios react-native
 */
appsRouter.post(
  '/',
  checkToken,
...
appsRouter.patch(
  '/:appName/updateInfo',
  checkToken,
  async (req: Req<{ appName: string }, AddAppConfig>, res, next) => {
    const { repositoryUrl, appKey, port } = req.body
    const { appName } = req.params
    const { logger } = req
    logger.info('try update app info', {
      repositoryUrl,
      appKey,
      port,
      appName,
    ...
    try {
      await appManager.updateAppInfo(req.users.id, appName, {
        repositoryUrl,
        appKey,
        port,
      })
      logger.info('update app info success', {
        repositoryUrl,
        appKey,
        port,
        appName,
      })
      const data = await appManager.findAppByName(req.users.id, appName)
      res.send({
        name: data.name,
        repositoryUrl: data.repository_url,
        appKey: data.native_app_key,
        port: data.port,
      })
    } catch (e) {
      if (e instanceof AppError) {
        logger.info('update app failed', {
          name: appName,
          error: e.message,
        })
