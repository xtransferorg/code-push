...
export const config = {
  ...
  qiniu: {
    ...
    innerDownloadUrl:
      process.env.QINIU_DOWNLOAD_URL || process.env.DOWNLOAD_URL,
  },
  ...
  s3: {
    ...
    innerDownloadUrl: process.env.AWS_DOWNLOAD_URL || process.env.DOWNLOAD_URL,
  },
  ...
  oss: {
    ...
    // binary files download host address
    downloadUrl: `https://${process.env.CPS_DOWNLOAD_HOST}/${process.env.OSS_PREFIX || ''}`,
    innerDownloadUrl: `${process.env.INNER_DOWNLOAD_URL}/${process.env.OSS_PREFIX || ''}`,
  },
  ...
  tencentcloud: {
    ...
    innerDownloadUrl: process.env.COS_DOWNLOAD_URL || process.env.DOWNLOAD_URL,
  },
  ...
  jwt: {
    ...
    tokenSecret:
      ...
      'E29FCFD0B9FF4AED4B2105C9FBB52D74741E7450A886C2CAFA1FA0A680CE923F',
  },
  common: {
     ...
     */
    tryLoginTimes: toNumber(process.env.TRY_LOGIN_TIMES, 0),
    ...
    updateCheckCache: process.env.USE_REDIS === 'on',
    // options value is (true | false), when it's true, it will cache rollout results in redis
    rolloutClientUniqueIdCache: toBool(
      process.env.ROLLOUT_CLIENT_UNIQUE_ID_CACHE,
    ),
    // 一个APP版本下的第一次热更新缓存目录，用于存放第一次热更新的文件，用于后续的热更新差异计算
    localCodePushDir: process.env.LOCAL_CODE_PUSH_DIR || os.tmpdir(),
  },
