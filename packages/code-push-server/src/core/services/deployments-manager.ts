...
class DeploymentsManager {
  getAllPackageIdsByDeploymentsId(deploymentsId: number) {
    return Packages.findAll({
      where: { deployment_id: deploymentsId },
      order: [['created_at', 'DESC']],
    })
  }

  existDeploymentName(appId: number, name: string) {
  ...
  addDeployment(name: string, appId: number, uid: number) {
    return Users.findByPk(uid).then((user) => {
      if (_.isEmpty(user)) {
        throw new AppError("can't find user")
      }
      return this.existDeploymentName(appId, name).then(() => {
  ...
  renameDeploymentByName(
    deploymentName: string,
    appId: number,
    newName: string,
  ) {
    return this.existDeploymentName(appId, newName).then(() => {
  ...
  deleteDeploymentByName(deploymentName: string, appId: number) {
  ...
  findDeploymentByName(deploymentName: string, appId: number, logger: Logger) {
  ...
  findDeploymentsPackages(deploymentsVersionsId) {
    return DeploymentsVersions.findOne({
      where: { id: deploymentsVersionsId },
    }).then((deploymentsVersionsInfo) => {
      if (deploymentsVersionsInfo) {
        return this.findPackagesAndOtherInfos(
          deploymentsVersionsInfo.current_package_id,
        )
      }
      return null
    })
  ...
  formatPackage(packageVersion: {
    ...
    return {
      ...
      manifestBlobUrl: getBlobDownloadUrl(
        packageVersion.packageInfo.manifest_blob_url,
      ),
  ...
  listDeployments(appId: number) {
    return Deployments.findAll({ where: { appid: appId } }).then(
      (deploymentsInfos) => {
  ...
  listDeployment(deploymentInfo) {
    return this.findDeploymentsPackages([
  ...
  async findDeploymentVersionWithAppVersion(
    appVersion: string,
    { uid, appName, deploymentName, logger }: any,
  ) {
    try {
      const deploymentId = (
        await deploymentsManager.findDeploymentByName(
          deploymentName,
          (await appManager.findAppByName(uid, appName)).id,
          logger,
        )
      ).id
      return DeploymentsVersions.findOne({
        where: { app_version: appVersion, deployment_id: deploymentId },
      })
    } catch (e) {
      logger.info('rollback check version id failed ' + e)
    }
  }
}
